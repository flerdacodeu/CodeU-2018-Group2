...
def check_input(start: List[int], final: List[int]) -> bool:
    ...
    # speed optimization, check lengths of sets are the same before checking sets
    if len(start_set) != len(car_set) or len(final_set) != len(car_set):
        return False
...
def _move(car_positions: List[int], car_num: int) -> int:
    ...
    Args:
        car_positions: list of integers where ith element is the current position of ith car
        car_num: int number of car to move
    Returns:
        int: new empty position
    """
    car_positions[0], car_positions[car_num] = car_positions[car_num], car_positions[0]
...
def generate_moves(start: List[int], final: List[int]) -> List[int]:
    ...
    Args:
        ...
        final: list of integers where ith element is the car that should be in the ith space
    Returns:
        List[int]: returns final list of positions (this should be equal to final and is only for testing)
    """
    ...
    for car, place in enumerate(car_positions):
        ...
        # move car that should end up in current empty space while such car exists
